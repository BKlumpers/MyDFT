...
const double angstrom2bohr = 1.88973; //allows conversion from Angstrom to Bohr (lengths should be native in Bohr)
...
int main() {
    ...
    vec3 pos2;
    pos2 << 0.0,0.0,1.4; //position second atom 1.4B away from the 1st atom (H-H bond length)
    vec3 pos3;
    pos3 << 0.0,0.0,2.116; //position third atom 2.116B away from the 1st atom (C-O bond length in C=O)
    ...
    if(HeHp_switch){
        ...
        vec3 pos_HeHp;
        pos_HeHp << 0.0,0.0,0.772*angstrom2bohr;
        ...
        if(visual){
            ...
            plot2D("HeHp.txt",height,width,xmin,xmax,zmin,zmax+pos_HeHp[3],AO_list_HeHp,HeHp_results.C_vector);
        }
    }
    ...
    if(HeH_switch){
        ...
        //HeH+ molecule
        cout << "Testcase for HeH (neutral): " << endl << endl;
        //create CGF for Helium
        CGF cgfHe_1p;
        cgfHe_1p.add_gto(0.15432897000000001,6.3624213899999997,0.0,0.0,0.0,pos);
        cgfHe_1p.add_gto(0.53532813999999995,1.1589229999999999,0.0,0.0,0.0,pos);
        ...
        //position hydrogen appropriate distance from helium:
        vec3 pos_HeHp;
        pos_HeHp << 0.0,0.0,3.86386*angstrom2bohr;
        CGF cgfH_He; //create orbitals for second hydrogen atom at pos2
        cgfH_He.add_gto(0.15432897000000001,3.4252509099999999,0.0,0.0,0.0,pos_HeHp);
        cgfH_He.add_gto(0.53532813999999995,0.62391373000000006,0.0,0.0,0.0,pos_HeHp);
        cgfH_He.add_gto(0.44463454000000002,0.16885539999999999,0.0,0.0,0.0,pos_HeHp);
        ...
        vector<int> nelec_list_HeHp; //create list of electrons for each atom
        nelec_list_HeHp.push_back(He_nelec); //He+ has one electron less
        nelec_list_HeHp.push_back(H_nelec);
        ...
        if(visual){
            //rescale z-axis to HeH+ bond length, then write image of ground-state MO
            plot2D("HeH.txt",height,width,xmin,xmax,zmin,zmax+pos_HeHp[3],AO_list_HeHp,HeHp_results.C_vector);
        }
    }
